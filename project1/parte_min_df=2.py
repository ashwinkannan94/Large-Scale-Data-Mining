...
plt.title('ROC Curve of LSI SVM Classification with Gamma=1000 and min_df=2')
...
plt.ylabel('True Positive Rate')
plt.xlabel('Flase Positive Rate')
plt.title('ROC Curve of LSI SVM Classification with Gamma=0.001 and min_df=2')
...
NMF_train_data = np.concatenate((nmf_tfidf[0:len(computer_train.data)], nmf_tfidf[(len(computer_train.data) +
                                                                                   len(computer_test.data)):(len(computer_train.data) + len(computer_test.data) + len(recreational_train.data))]))
svm_classifier = svm.SVC(kernel='linear', gamma=1000, random_state=42)
svm_classifier.fit(NMF_train_data, train_classification)

class_which_was_predicted = svm_classifier.predict(NMF_test_data)
actual_class_passed = test_classification
score_received = svm_classifier.decision_function(NMF_test_data)
...
false_positive_rate_NMF, true_positive_rate_NMF, c = roc_curve(actual_class_passed, score_received)
plt.figure(3)
plt.plot(false_positive_rate_NMF, true_positive_rate_NMF)
plt.plot([0, 1], [0, 1])
plt.ylabel('True Positive Rate')
plt.xlabel('Flase Positive Rate')
plt.title('ROC Curve of NMF SVM Classification with Gamma=1000 and min_df=2')

print('Setting gamma to 0.001')
svm_classifier = svm.SVC(kernel='linear', C=0.001, random_state=42)
svm_classifier.fit(NMF_train_data, train_classification)
...
plt.ylabel('True Positive Rate')
plt.xlabel('Flase Positive Rate')
plt.title('ROC Curve of NMF SVM Classification with Gamma=0.001 and min_df=2')
plt.show()
